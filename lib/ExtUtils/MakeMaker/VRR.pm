...
our $VERSION = '0.01';
our @ISA = qw(Exporter);
our @EXPORT = qw(vrr2text var2vrr rule2vrr text2vrrs);
...
sub rule2vrr {
    my ($self, $target, $append, $deps, $recipes) = @_;
    +{
        type => 'rule',
        left => $target,
        right => $deps,
...
sub text2vrrs {
    ...
    VRR: while (defined($line = _getline $text)) {
        if ($line eq '') {
            ...
            push @m, $self->var2vrr($var, length($value) ? ($value) : ());
            ...
            push @m, $self->rule2vrr($target, $colons eq '::', \@deps, \@recipes);
